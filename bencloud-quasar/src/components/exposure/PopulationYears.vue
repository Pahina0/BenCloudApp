...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    watch(
      () => store.state.exposure.populationYears,
    ...
    watch(
      ...
      (currentSelectedItem, prevSelectedItem) => {
        if (currentSelectedItem != prevSelectedItem) {
          store.commit("exposure/updateApplyYearsToAll", currentSelectedItem);
        }
      }
    );

    onBeforeMount(() => {
      applyAll.value = store.state.exposure.applyYearsToAll;
      scenarios.value = [];
      for(let i = 0; i < store.state.exposure.postPolicyAirQualityName.length; i++) {
        scenarios.value.push({index: i, name: store.state.exposure.postPolicyAirQualityName[i].name, years: store.state.exposure.postPolicyAirQualityName[i].years});
      }
    });
    ...
    onUpdated(() => {
      ...
      if(items.length) {
        ...
        store.commit("exposure/updatePostPolicyAirQualityName", yearsArr);
        console.log(store.state.exposure.postPolicyAirQualityName);
      }
    })();
